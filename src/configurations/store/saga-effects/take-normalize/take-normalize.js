...
const takeNormalize = (patternOrChannel, saga, ...sagaArgs) =>
  fork(
    ...
    function*() {
      const sagaForksById = {};
      const sagaFilter = sagaActionId =>
        !sagaForksById[sagaActionId] || !sagaForksById[sagaActionId].isRunning();
      const sagaAssign = (sagaFork, sagaActionId) => {
        sagaForksById[sagaActionId] = sagaFork;
      };
      while (true) {
